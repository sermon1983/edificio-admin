...
export default function Incidentes() {
  const [incidentes, update] = useStore(s => s.incidentes)
  ...
  const filtered = incidentes.filter(i => {
    ...
    const matchQ = i.titulo.toLowerCase().includes(q) || i.reportado_por.toLowerCase().includes(q)
    const matchE = filtroEstado === 'Todos' || i.estado === filtroEstado
    const matchP = filtroPrioridad === 'Todas' || i.prioridad === filtroPrioridad
    return matchQ && matchE && matchP
  })

  function save() {
    if (!form.titulo || !form.fecha) return
    update('incidentes', is => [...is, { ...form, id: nextId(is) }])
    setModal(false); setForm(EMPTY)
  }

  function del(id) { update('incidentes', is => is.filter(i => i.id !== id)); setDetail(null) }

  function changeEstado(id, est) {
    update('incidentes', is => is.map(i => i.id===id ? { ...i, estado:est } : i))
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div><h1>Gestión de Incidentes</h1><p>Reportes y seguimiento de eventos</p></div>
      ...
      <div className="grid-4" style={{ marginBottom:20 }}>
        {ESTADOS.map(e => (
          <div key={e} className="card" style={{ padding:'14px 18px', cursor:'pointer' }} onClick={() => setFiltroEstado(filtroEstado===e?'Todos':e)}>
            ...
            <p style={{ fontSize:26, fontWeight:700, color:'var(--text-primary)', marginTop:3 }}>
              {incidentes.filter(i=>i.estado===e).length}
            </p>
          </div>
        ))}
      </div>
      ...
      <div className="filters-row">
        <div className="search-bar">
          <Search size={15} />
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar incidente..." />
        </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.length === 0 ? (
          <div className="card"><div className="empty-state"><AlertTriangle size={32} /><p>Sin incidentes registrados</p></div></div>
        ) : filtered.map(i => (
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget && setDetail(null)}>
          <div className="modal">
            <div className="modal-header">
              <div>
                <h3 style={{ marginBottom:6 }}>{detail.titulo}</h3>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px 20px', marginBottom:16 }}>
                {[['Reportado por', detail.reportado_por], ['Fecha', detail.fecha], ['Estado', detail.estado]].map(([k,v])=>(
                  <div key={k}>
                    ...
                    <p style={{ color:'var(--text-primary)', fontWeight:500, fontSize:14 }}>{v}</p>
                  </div>
                ))}
              </div>
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget && setModal(false)}>
          <div className="modal">
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={save}>Crear Incidente</button>
            </div>
          </div>
        </div>
      )}
