...
export default function OrdenesTrabajos() {
  ...
  const filtered = ordenes.filter(o =>
    ((o.titulo||'').toLowerCase().includes(search.toLowerCase()) || (o.asignado_a||'').toLowerCase().includes(search.toLowerCase())) &&
  ...
  async function setEstado(id, est) {
    try { await update(id, { estado: est }) } catch(e) { alert('Error: '+e.message) }
  }

  const prioColor = { Alta:'badge-red', Media:'badge-yellow', Baja:'badge-green' }
  const estColor  = { Pendiente:'badge-yellow', 'En Proceso':'badge-blue', Completada:'badge-green', Cancelada:'badge-red' }
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div><h1>Órdenes de Trabajo</h1><p>Mantenimiento y reparaciones</p></div>
        <div style={{ display:'flex', gap:10 }}>
          <div style={{ display:'flex', background:'var(--bg-surface)', border:'1px solid var(--border-subtle)', borderRadius:8, overflow:'hidden' }}>
            {['kanban','list'].map(v=>(
              <button key={v} onClick={()=>setView(v)} style={{ padding:'7px 14px', border:'none', cursor:'pointer', fontSize:12.5, fontWeight:500, fontFamily:'var(--font-sans)', transition:'all 0.15s', background:view===v?'var(--accent)':'transparent', color:view===v?'white':'var(--text-muted)' }}>
                {v==='kanban'?'Kanban':'Lista'}
              </button>
            ))}
          </div>
          <button className="btn btn-primary" onClick={()=>{setForm(EMPTY);setModal(true)}}><Plus size={16}/> Nueva Orden</button>
        </div>
      </div>
      ...
      <div className="filters-row">
        <div className="search-bar"><Search size={15}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar orden o técnico..."/></div>
      ...
      {view==='kanban' ? (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16 }}>
          {['Pendiente','En Proceso','Completada'].map(col=>(
            <div key={col}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, padding:'6px 10px', background:'var(--bg-surface)', border:'1px solid var(--border-subtle)', borderRadius:8 }}>
                <span className={`badge ${estColor[col]}`}>{col}</span>
                <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:12, color:'var(--text-muted)' }}>{filtered.filter(o=>o.estado===col).length}</span>
              </div>
        ...
        <div className="card" style={{ padding:0, overflow:'hidden' }}>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Título</th><th>Categoría</th><th>Asignado a</th><th>Prioridad</th><th>F. Creación</th><th>F. Límite</th><th>Estado</th><th></th></tr></thead>
              <tbody>
                {filtered.length===0
      ...
      {modal && (
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div className="modal">
            <div className="modal-header"><h3>Nueva Orden de Trabajo</h3>
            ...
            <div className="modal-body">
              ...
              <div className="grid-2">
                <div className="form-group"><label>Categoría</label>
                  <select className="form-control" value={form.categoria} onChange={e=>setForm(f=>({...f,categoria:e.target.value}))}>{CATEGORIAS.map(c=><option key={c}>{c}</option>)}</select>
                </div>
                ...
                <div className="form-group" style={{ gridColumn:'span 2' }}><label>Asignado a</label>
                ...
                <div className="form-group"><label>Fecha Creación *</label>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={save} disabled={saving}>{saving?'Guardando...':'Crear Orden'}</button>
            </div>
          </div>
        </div>
      )}
...
function OrdCard({ o, prioColor, onDel, onEstado }) {
  return (
    <div className="card" style={{ padding:'14px 16px' }}>
      ...
      {o.asignado_a && <p style={{ marginTop:8, fontSize:12, color:'var(--text-muted)' }}>👷 {o.asignado_a}</p>}
      {o.fecha_limite && (
